...
(function () {
  ...
  function render() {
    ...
    var sess = (state.screen === "hub" || state.screen === "clubs" || state.screen === "me" || state.screen === "explore") ? null : currentSession();
    var mode = (state.screen === "clubs" || state.screen === "me" || state.screen === "explore") ? "top" : state.screen === "hub" ? "hub" : (sess && sess.kind === "info" ? "info" : sess && sess.kind === "match" ? "match" : "app");
    ...
    if (mode === "top") { appEl.classList.add("lvl-top"); $("#gate").classList.add("hidden"); renderTopHeader(); renderTopNav(); setChrome(false); main.innerHTML = state.screen === "me" ? viewMeTop() : state.screen === "explore" ? viewExplore() : viewClubs(); window.scrollTo(0, 0); return; }
  ...
  function renderTopNav() {
    var tabs = [["clubs", "home", "홈"], ["explore", "compass", "탐색"], ["me", "user", "마이"]];
  ...
  function viewExplore() {
    ...
    if (Store.mode === "demo") h += '<div class="demo-note">⚠️ <b>오프라인 임시 모드</b> — 실시간 연결이 안 돼, 입력 내용이 이 기기에만 저장돼요.</div>';
    h += '<div class="hub-head"><h1>탐색</h1><p class="hub-sub">관심 있는 동호회를 둘러보고 가입해보세요</p></div>';
    if (!list.length) return h + '<div class="empty-msg">아직 공개된 동호회가 없어요.</div></div>';
    h += '<div class="list-grid sess-grid">';
  ...
  function hubBoard(club) {
    ...
    var segs = [["notice", "공지사항"], ["poll", "투표"], ["dues", "회비"]];
  ...
  function boardNotices(club) {
    ...
    if (mng) h += '<button class="btn-pri btn-block" data-action="add-club-notice" style="margin-bottom:12px">' + icon("plus", 16) + " 공지 작성</button>";
    if (!list.length) return h + '<div class="empty-msg">아직 공지가 없어요.' + (mng ? "" : " 운영진이 공지를 올리면 여기 표시돼요.") + "</div>";
  ...
  function formClubNotice() {
    openModal('<h2>공지 작성</h2><label>내용</label><textarea id="cn-text" rows="4" placeholder="공지 내용을 입력하세요"></textarea>' +
      '<label class="chk"><input type="checkbox" id="cn-pin"> 상단 고정</label>' +
      '<div class="modal-foot"><button class="btn-line" data-action="close-modal">취소</button><button class="btn-pri" data-action="save-club-notice">등록</button></div>');
  }
  ...
  function formAddClub(editId) {
    ...
    openModal("<h2>" + (ed ? "동호회 수정" : "동호회 개설") + "</h2>" +
      ...
      '<label>동호회 이름</label><input id="f-cname" placeholder="예: 강남 3구 당구 동호회" value="' + (ed ? esc(ed.name || "") : "") + '">' +
  ...
  function sessionSkillCard(club) {
    ...
    var h = '<div class="card col"><div class="ms-row"><span>' + icon("ballot", 16) + ' 이 세션 ' + esc(label) + ' 기록</span>' + (n ? '<span class="ms-amt" style="font-size:12px">' + n + '건</span>' : '') + "</div>";
    ...
    h += '<button class="link" data-action="go-club-ranking" style="display:block;width:100%;text-align:center;margin-top:10px">동호회 순위 보기 ›</button>';
    return h + "</div>";
  }
  function viewHome() {
    var t = tripMeta();
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "save-club-notice") { var cnT = clampStr(($("#cn-text") || {}).value, 1000); if (!cnT) return; Store.set("clubnotices/" + state.clubId + "/" + key(), { text: cnT, by: me, pinned: !!($("#cn-pin") || {}).checked, ts: Date.now() }); closeModal(); render(); return; }
    if (a === "del-club-notice") { var cnk = t.getAttribute("data-id"), cno = (obj(DB.clubnotices)[state.clubId] || {})[cnk]; if (!cno || !(cno.by === me || canManage(me))) return; if (confirm("이 공지를 삭제할까요?")) { Store.remove("clubnotices/" + state.clubId + "/" + cnk); render(); } return; }
